...
export interface ConstOptions<K, V> {
  ...
  upsert?: ((key: K, additionalArgs?: unknown[]) => UpsertResult<V> | Promise<UpsertResult<V>>) | undefined
...
/** Main interface */
export class LRU_TTL<K, V> implements NodeChain {
  ...
  get(key: K, upsert?: boolean, additionalUpsertCbArgs?: unknown[]): V | Promise<V> | undefined {
  ...
  *values(): IterableIterator<V | Promise<V>> {
  ...
  /** ForEach */
  forEach(cb: (value: V | Promise<V>, key: K, cache: this) => void, thisArg: unknown) {
    var it = this._map.values()
    var p = it.next()
